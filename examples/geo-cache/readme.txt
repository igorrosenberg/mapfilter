...
cat liste_villes.csv | sed 's/"//g' | cut -f 2,7,8 -d ';' | sed 's/;/\&latitude=/'| sed 's/;/\&longitude=/' | sed 's|^|sleep 0.02 ; wget -O - -o /tmp/out "http://igor.rosenberg.free.fr/gcm/latest/geo-cache/update.php?pass=update_key\&address=|' | sed 's/$/"/' | sh  

# batch check latitude & longitude
tac liste_villes.csv | sed 's/"//g' | cut -f 2,7,8 -d ';' | sed 's/;/\&extra1=/'| sed 's/;/\&extra2=/' | sed 's|^|sleep 0.02 ; wget -O - -o /tmp/out "http://igor.rosenberg.free.fr/gcm/latest/geo-cache/cache.php?pass=update_key\&address=|' | sed 's/$/"/' | sh
